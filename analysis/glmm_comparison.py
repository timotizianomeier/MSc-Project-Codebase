...
per participant, and put its Wald p-values next to the pipeline's existing
nonparametric/t twins:
...
interaction p as well.  NB with two observations per participant the
Gaussian LMM interaction is the model-based analogue of the delta
comparison, so agreement is the expected outcome; the LMM uses all
available sessions (incl. unpaired ones).

Run:  .venv/bin/python glmm_comparison.py   for the terminal summary +
output/glmmdoc.tex (compile with pdflatex for the comparison PDF).
generate_results.py imports glmm_rows() for the thesis fragment.
"""
import os
import sys
import warnings
...
def glmm_rows(groups) -> list[dict]:
    """One dict per session-level metric: LMM/GEE p-values plus the
    pipeline's nonparametric and t twins ('label' short, 'metric' full)."""
    ...
    for label, df, dec in gr._cross_condition_rows(groups, None):
        ...
        # effects coding (+-0.5): the 'main' coefficients then estimate
        # each factor's effect averaged over the other (a 0/1-coded model
        # with the interaction present would give simple effects at the
        # reference level instead); the interaction is coding-invariant.
        ...
        m_a = pooled.dropna()[lambda s:
                              s.index.map(groups.get) == gr.GROUP_ADHD]
        m_c = pooled.dropna()[lambda s:
                              s.index.map(groups.get) == gr.GROUP_CONTROL]
        ...
        p_cond_w = (safe(lambda r, c: sps.wilcoxon(r, c),
                    ...
                    if len(paired) > 1 and (delta != 0).any()
                    else float("nan"))
        ...
        try:
            with warnings.catch_warnings():
                # boundary warnings: random-intercept variance -> 0 for a
                # couple of metrics, expected with 2 obs per participant
        ...
        if label in COUNT_METRICS:
            try:
                gee = smf.gee("value ~ robot * adhd", groups="pid",
...
def main() -> None:
    ...
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           "output")  # gitignored, like the fragments
    ...
    hdr = (f"{'metric':<42} {'n':>6} | {'int LMM':>8} {'GEE':>6} "
           f"{'pU(d)':>6} {'pt(d)':>6} | {'cond LMM':>8} {'pW':>6} "
           f"{'pt':>6} | {'grp LMM':>8} {'pU':>6} {'pt':>6}")
    print("\n" + hdr + "\n" + "-" * len(hdr))
    for r in rows:
        print(f"{r['metric'][:42]:<42} {r['n']:>6} | "
              f"{_fmt_p(r['p_int_lmm']):>8} {_fmt_p(r['p_int_gee']):>6} "
              f"{_fmt_p(r['p_int_u']):>6} {_fmt_p(r['p_int_t']):>6} | "
              f"{_fmt_p(r['p_cond_lmm']):>8} {_fmt_p(r['p_cond_w']):>6} "
              f"{_fmt_p(r['p_cond_t']):>6} | "
              f"{_fmt_p(r['p_grp_lmm']):>8} {_fmt_p(r['p_grp_u']):>6} "
              f"{_fmt_p(r['p_grp_t']):>6}"
    ...
    body = "\n".join(
        f"{r['metric']} & {r['n']} & "
        + " & ".join(("{--}" if r[k] != r[k] else "{$<.001$}"
                      if r[k] < 0.001 else f"{r[k]:.3f}")
                     for k in ("p_int_lmm", "p_int_gee", "p_int_u",
                               "p_int_t", "p_cond_lmm", "p_cond_w",
                               "p_cond_t", "p_grp_lmm", "p_grp_u",
                               "p_grp_t"))
        + " \\\\" for r in rows)
...
GEE = Poisson generalized estimating equations with robust SEs,
intervention counts only.  $n$ = sessions/participants entering the LMM
(unpaired sessions included; the nonparametric twins use their usual
pairings).  Signal metrics carry the speech exclusion.
\vspace{0.8em}
